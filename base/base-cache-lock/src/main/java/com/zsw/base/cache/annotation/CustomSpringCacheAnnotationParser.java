...
import com.zsw.base.cache.interceptor.CacheLockOperation;
...
/**
 * @author ZhangShaowei on 2017/10/12 10:19
 */
...
public class CustomSpringCacheAnnotationParser implements CacheAnnotationParser, Serializable {
    ...
    protected Collection<CacheOperation> parseCacheAnnotations(DefaultCacheConfig cachingConfig, AnnotatedElement ae) {
        ...
        Collection<CacheLock> cacheLocks = AnnotatedElementUtils.getAllMergedAnnotations(ae, CacheLock.class);
        if (!cacheLocks.isEmpty()) {
            ops = lazyInit(ops);
            for (CacheLock cacheLock : cacheLocks) {
                ops.add(parseCacheLockAnnotation(ae, cachingConfig, cacheLock));
            }
        }
    ...
    CacheLockOperation parseCacheLockAnnotation(
            AnnotatedElement ae, DefaultCacheConfig defaultConfig, CacheLock cacheLock) {
        ...
        builder.setTimeOut(cacheLock.timeOut());
        builder.setExpireTime(cacheLock.expireTime());
