package com.zsw.base.commons.utils;
...
public class OkHttpUtils {
    ...
    public static void callSuccess(Request request) {
        try {
            log.info("callSuccess:url:{}", request.url().url().toString());
    ...
    public static Response callForResponse(Request request) {
        try {
            log.info("callForResponse:url:{}", request.url().url().toString());
    ...
    public static String callForString(Request request) {
        try {
            log.info("callForString:url:{}", request.url().url().toString());
            Response response = CLIENT.newCall(request).execute();
            String result = response.body().string();
            log.info("callForString:result:{}", result == null ? null : result.length() > 250 ?
    ...
    public static byte[] callForBytes(Request request) {
        try {
            log.info("callForBytes:url:{}", request.url().url().toString());
            Response response = CLIENT.newCall(request).execute();
