...
/**
 * @author Shaowei Zhang on 2019/4/24 0:59
 **/
@Data
public class EntityOperationImpl<T> implements EntityOperation<T> {
    ...
    public EntityOperationImpl(Class<T> clazz) {
        this.clazz = clazz;
    ...
    public String findAll() {
        return this.findByParams(null).getSql();
    }
     ...
     */
    @Override
    public PropertiesHandler findByParams(T t) {
        // 属性转换成
        Map<String, Object> objectMap = this.readProperties(t);
        // 过滤 null 的属性
        objectMap.entrySet().removeIf(next -> next.getValue() == null);
        return new PropertiesHandler(this.findAllSql(objectMap.keySet()), objectMap);
    }

    private String findAllSql(Set<String> params) {
        String sql = "select * from " + this.table;
        if (!CollectionUtils.isEmpty(params)) {
            StringBuilder sb = new StringBuilder(" where 1=1 ");
            for (String param : params) {
                sb.append("and ").append(param).append("=? ");
            }
            sql += sb.toString();
        }
        return sql;
     ...
     * @return
     */
    private Map<String, Object> readProperties(T t) {
