package com.zsw.quartz.controller;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.google.gson.reflect.TypeToken;
import com.zsw.quartz.exception.BizException;
import com.zsw.quartz.persistence.entity.SystemJob;
import com.zsw.quartz.pojo.LayuiData;
import com.zsw.quartz.service.SystemJobService;
import com.zsw.quartz.utils.Constant;
import com.zsw.quartz.utils.SchedulerUtil;
...
public class JobController {
    ...
    public String toDetail(Integer id, Model model) {
        SystemJob job = sysJobService.selectByPrimaryKey(id);
    ...
    public LayuiData queryJobList(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (StringUtils.hasText(jobClassPath)) {
            map.put("jobClassPath", jobClassPath);
        }
        if (StringUtils.hasText(jobDescribe)) {
    ...
    public int addjob(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.isEmpty(jobClassPath)) {
        ...
        if (StringUtils.hasText(jobDataMap)) {
            try {
                Map<String, Object> map = this.gson.fromJson(jobDataMap, new TypeToken<Map<String, Object>>() {
                }.getType());
        ...
        // 已存在校验
        SystemJob queryBean = new SystemJob();
        queryBean.setJobName(jobName);
        SystemJob result = sysJobService.selectByBean(queryBean);
        if (null != result) {
            throw new BizException("任务名为" + jobName + "的任务已存在！");
        }

        SystemJob bean = new SystemJob();
    ...
    public int changeState(@RequestParam(value = "id") String idStr) throws Exception {
        ...
        SystemJob updateBean = new SystemJob();
        updateBean.setId(id);
        //如果是现在是启用，则停用
    ...
    public int updateByBean(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        SystemJob result = sysJobService.selectByPrimaryKey(id);
        // 数据不存在
        if (null == result) {
